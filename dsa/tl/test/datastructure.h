...
template <typename T> struct extract_first_type {
  using type = std::tuple_element_t<0, T>;
};
template <typename T>
using extract_first_type_t = typename extract_first_type<T>::type;

template <typename T> struct add_index {
  template <std::size_t I> static auto make() {
...
// 带索引的元组构造器
template <typename T, typename Index> struct make_indexed_tuple {
...
// 带标签的类型构造器
template <typename T, typename Tag> struct make_tagged_type {
...
// 用于提取和修改元组的模板
template <typename T> struct extract_and_modify {
  using type = T; // 默认实现
};

template <typename T, std::size_t I>
struct extract_and_modify<
    std::tuple<T, std::integral_constant<std::size_t, I>>> {
  // 提取原始类型并添加新的元数据
  using type = std::tuple<T, std::integral_constant<std::size_t, I>,
                          std::integral_constant<std::size_t, I * 2>>;
};
...
// 可配置的转换器
template <typename Extra1, typename Extra2> struct configurable_transformer {
...
// 添加字段信息的模板
template <typename T> struct add_field_info {
  using type = std::tuple<T, field_info>;
};
template <typename T> using add_field_info_t = typename add_field_info<T>::type;

// 添加类型特性的模板
template <typename T> struct add_type_traits {
...
// 带索引的序列化模板
template <typename T, std::size_t Index> struct serialize_with_index {
...
// 用于应用序列化的模板
template <typename Tuple> struct apply_serialize;
...
// 累积偏移量序列化模板
template <typename Tuple> struct apply_accumulated_serialize;

template <typename T, typename Index, typename Offset>
struct apply_accumulated_serialize<std::tuple<T, Index, Offset>> {
  using type =
      std::tuple<T, std::integral_constant<std::size_t, Index::value>,
                 std::integral_constant<std::size_t, sizeof(T)>, Offset>;
};

template <typename Tuple>
using apply_accumulated_serialize_t =
    typename apply_accumulated_serialize<Tuple>::type;

template <std::size_t... Is>
auto make_index_list(std::index_sequence<Is...>)
    -> tl::type_list<std::integral_constant<std::size_t, Is>...>;

template <typename T> struct is_even_value : std::false_type {};

template <int N>
struct is_even_value<int_type<N>> : std::bool_constant<N % 2 == 0> {};

template <typename T> struct is_odd_value : std::false_type {};

template <int N>
struct is_odd_value<int_type<N>> : std::bool_constant<N % 2 == 1> {};

template <typename T>
struct is_type_a_or_c
    : std::bool_constant<std::is_same_v<T, A> || std::is_same_v<T, C>> {};
...
template <int N> struct is_int_type<int_type<N>> : std::true_type {};

template <typename T>
struct is_a_or_b
    : std::bool_constant<std::is_same_v<T, A> || std::is_same_v<T, B>> {};

template <typename T>
struct is_not_b : std::bool_constant<!std::is_same_v<T, B>> {};

template <typename T>
struct is_replacable
    : std::bool_constant<std::is_same_v<T, B> || std::is_same_v<T, D>> {};
...
template <class Acc, class T> struct acc_size {
  using type = std::integral_constant<std::size_t, Acc::value + sizeof(T)>;
};

template <class Acc, class T> struct make_tuple_list {
...
struct Counter {
  int count = 0;
  template <typename T> void operator()(detail::identity<T>) { ++count; }
};

struct sum_calculator {
  int sum = 0;
  template <typename T> void operator()(detail::identity<T>) {
    sum += T::value;
  }
};

template <typename Acc, typename T> struct sum_accumulator {
  using type = int_type<Acc::value + T::value>;
};

// 字符串长度累积
template <typename Acc, typename T> struct type_name_length {
