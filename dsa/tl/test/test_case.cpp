...
TEST(type_list, find_and_count) {
  ...
  CHECK_COMPILE_TIME(List2::find_first_if<is_pointer> ==
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<UniqueList, tl::type_list<int, double, char>>);

  using Reversed = List2::reverse_t;
  CHECK_COMPILE_TIME(
      std::is_same_v<Reversed, tl::type_list<float, char, double, int>>);
  return true;
}
...
TEST(type_list, filter_operations) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<Replaced, tl::type_list<int, long double, char, float>>);
...
TEST(type_list, concatenation) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<Appended,
                     tl::type_list<int, double, char, float, void *>>);

  using Prepended = List4::prepend_t<bool>;
  CHECK_COMPILE_TIME(
      std::is_same_v<Prepended, tl::type_list<bool, int, double, char, float>>);

  using Transformed = List4::transform_t<std::add_pointer_t>;
  CHECK_COMPILE_TIME(
      std::is_same_v<Transformed,
                     tl::type_list<int *, double *, char *, float *>>);
  return true;
}
...
TEST(type_list, conversions) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<FromTuple0, tl::type_list<int, double, char>>);
...
TEST(type_list, zip_unzip) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename Zipped::at_t<0>, std::tuple<A, int, char>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename Zipped::at_t<1>, std::tuple<B, double, short>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<Unzipped1, tl::type_list<int, double, float>>);
...
TEST(type_list, flatten) {
  ...
  using Nested2 =
      tl::type_list<std::tuple<A, B>, tl::type_list<C, D>, std::variant<E, F>>;
...
TEST(type_list, relations) {
  ...
  using List10 =
      tl::type_list<int, int, int, double, double, char, float, long>;
  using List11 =
      tl::type_list<int, int, int, double, double, char, float, long>;
  ...
  CHECK_COMPILE_TIME(List5::is_subset_of<tl::type_list<int, double, char>> ==
                     true);
  CHECK_COMPILE_TIME(
      List5::is_subset_of<tl::type_list<int, double, char, float>> == true);
  ...
  CHECK_COMPILE_TIME(EmptyList::is_subset_of<tl::type_list<int, double>> ==
                     true);

  using SingleList = tl::type_list<int>;
  CHECK_COMPILE_TIME(tl::type_list<int>::equal_to<SingleList> == true);
  CHECK_COMPILE_TIME(tl::type_list<int>::equal_to<tl::type_list<double>> ==
                     false);
  CHECK_COMPILE_TIME(tl::type_list<int>::is_subset_of<SingleList> == true);
  CHECK_COMPILE_TIME(
      tl::type_list<int>::is_subset_of<tl::type_list<int, double>> == true);
  ...
  CHECK_COMPILE_TIME(tl::type_list<int, double, char>::is_subset_of<List5> ==
                     true);
  CHECK_COMPILE_TIME(tl::type_list<int, double, char>::is_subset_of<List8> ==
                     true);
  CHECK_COMPILE_TIME(tl::type_list<int, double, char>::is_subset_of<List9> ==
                     true);
...
TEST(type_list, iteration) {
  ...
  List::reverse_for_each(
      [&values](auto id) { values.push_back(decltype(id)::type::value); });
...
TEST(type_list, edge_cases) {
  ...
  // 超大列表测试（编译期压力测试）
  using LargeList =
      tl::type_list<int_type<1>, int_type<2>, int_type<3>, int_type<4>,
                    int_type<5>, int_type<6>, int_type<7>, int_type<8>,
                    int_type<9>, int_type<10>>;
  CHECK_COMPILE_TIME(LargeList::size == 10);
  CHECK_COMPILE_TIME(LargeList::count_if<is_int_type> == 10);

  // 类型转换边界
  using ComplexList =
      tl::type_list<std::tuple<A, B>, std::variant<C, D>, tl::type_list<E, F>>;
...
TEST(type_list, accumulate) {
  ...
  using SizeResult =
      detail::accumulate<type_name_length, std::integral_constant<size_t, 0>, A,
                         B, C>::type;
  ...
  using SizeResult0 =
      List1::accumulate_t<acc_size, std::integral_constant<std::size_t, 0>>;
  CHECK_COMPILE_TIME(SizeResult0::value == sizeof(int) + sizeof(double) +
                                               sizeof(char) + sizeof(float));

  using Result = detail::accumulate<make_tuple_list, tl::type_list<>, int,
                                    double, char>::type;
  // clang-format off
    CHECK_COMPILE_TIME(
...
TEST(type_list, comprehensive_operations) {
  ...
  // 测试用例2: 类型转换和映射
  {
    ...
    CHECK_COMPILE_TIME(
        std::is_same_v<PointerList, tl::type_list<int *, double *>>);

    using ConstList = List::transform_t<std::add_const_t>;
    CHECK_COMPILE_TIME(
        std::is_same_v<ConstList, tl::type_list<const int, const double>>);
  }
...
TEST(type_list, advanced_find_operations) {
  // 测试 find_last_if
  using List1 = tl::type_list<int_type<1>, int_type<2>, int_type<3>,
                              int_type<4>, int_type<2>>;
  ...
  using List3 = tl::type_list<int_type<2>, int_type<4>, int_type<6>,
                              int_type<3>, int_type<8>>;
  constexpr size_t first_not_even_index =
      List3::find_first_not_if<is_even_value>;
  CHECK_COMPILE_TIME(first_not_even_index == 3); // 第一个非偶数在索引3（值3）

  // 全部满足条件的情况
  using List4 =
      tl::type_list<int_type<2>, int_type<4>, int_type<6>, int_type<8>>;
  ...
  // 测试 find_last_not_if
  using List5 = tl::type_list<int_type<2>, int_type<3>, int_type<5>,
                              int_type<8>, int_type<10>>;
  ...
  CHECK_COMPILE_TIME(EmptyList::find_first_not_if<is_even_value> ==
                     static_cast<size_t>(-1));

  // 边界情况：单元素列表
  using SingleEven = tl::type_list<int_type<2>>;
  CHECK_COMPILE_TIME(SingleEven::find_first_not_if<is_even_value> ==
                     static_cast<size_t>(-1));
  CHECK_COMPILE_TIME(SingleEven::find_last_not_if<is_even_value> ==
                     static_cast<size_t>(-1));
  ...
  // 查找第一个不是A或C的类型
  constexpr size_t first_not_a_or_c =
      MixedList::find_first_not_if<is_type_a_or_c>;
  CHECK_COMPILE_TIME(first_not_a_or_c == 1); // B在索引1

  // 查找最后一个不是A或C的类型
  constexpr size_t last_not_a_or_c =
      MixedList::find_last_not_if<is_type_a_or_c>;
  CHECK_COMPILE_TIME(last_not_a_or_c == 4); // int_type<3>在索引4

  // 测试查找不到的情况
  using AllAOrC = tl::type_list<A, C, A, C>;
  CHECK_COMPILE_TIME(AllAOrC::find_first_not_if<is_type_a_or_c> ==
                     static_cast<size_t>(-1));
  CHECK_COMPILE_TIME(AllAOrC::find_last_not_if<is_type_a_or_c> ==
                     static_cast<size_t>(-1));
...
TEST(type_list, combined_operations) {
  ...
  // 场景1：找到第一个可替换元素并替换它
  constexpr size_t first_replacable =
      OriginalList::find_first_if<is_replacable>;
  CHECK_COMPILE_TIME(first_replacable == 1); // B在索引1

  using ReplacedFirst =
      typename OriginalList::replace_at_t<first_replacable, X>;
  CHECK_COMPILE_TIME(std::is_same_v<typename ReplacedFirst::at_t<1>, X>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ReplacedFirst::at_t<3>, D>); // D没有被替换
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ReplacedLast::at_t<1>, B>); // B没有被替换

  // 场景3：在第一个不可替换元素前插入新元素
  constexpr size_t first_not_replacable =
      OriginalList::find_first_not_if<is_replacable>;
  CHECK_COMPILE_TIME(first_not_replacable == 0); // A在索引0

  using InsertedBeforeFirst =
      typename OriginalList::insert_at_t<first_not_replacable, X>;
  ...
  constexpr size_t last_not_replacable =
      OriginalList::find_last_not_if<is_replacable>;
  CHECK_COMPILE_TIME(last_not_replacable == 4); // E在索引4

  using InsertedAfterLast =
      typename OriginalList::insert_at_t<last_not_replacable + 1, X>;
  ...
  using NumberList = tl::type_list<int_type<1>, int_type<2>, int_type<3>,
  ...
  using ReplacedEven =
      typename NumberList::replace_at_t<first_even, int_type<99>>;
...
TEST(type_list, extended_zip_operations) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedLists::at_t<0>, std::tuple<A, D>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedLists::at_t<1>, std::tuple<B, E>>);

  // 测试 type_list 与 std::tuple 的 zip
  using ZippedWithTuple = typename List1::zip_t<Tuple1>;
  CHECK_COMPILE_TIME(ZippedWithTuple::size == 3);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedWithTuple::at_t<0>, std::tuple<A, int>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedWithTuple::at_t<1>, std::tuple<B, double>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedWithVariant::at_t<0>, std::tuple<A, char>>);
  CHECK_COMPILE_TIME(std::is_same_v<typename ZippedWithVariant::at_t<1>,
                                    std::tuple<B, short>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedMixed::at_t<0>, std::tuple<A, int, char>>);
  CHECK_COMPILE_TIME(std::is_same_v<typename ZippedMixed::at_t<1>,
                                    std::tuple<B, double, short>>);
  CHECK_COMPILE_TIME(std::is_same_v<typename ZippedMixed::at_t<2>,
                                    std::tuple<C, float, long>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedSameTypes::at_t<0>, std::tuple<A, A, A>>);
  return true;
}
...
TEST(type_list, extended_unzip_operations) {
  ...
  using TupleList =
      tl::type_list<std::tuple<A, int, char>, std::tuple<B, double, short>,
                    std::tuple<C, float, long>>;
  ...
  using VariantList =
      tl::type_list<std::variant<A, B>, std::variant<int, double, char>,
                    std::variant<char, short>>;

  using UnzippedVariants = typename VariantList::unzip_t;
  CHECK_COMPILE_TIME(std::tuple_size_v<UnzippedVariants> ==
                     2); // 每个variant有2个类型
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename FirstVariantUnzipped::at_t<1>, int>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename FirstVariantUnzipped::at_t<2>, char>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename SecondVariantUnzipped::at_t<0>, B>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename SecondVariantUnzipped::at_t<1>, double>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename SecondVariantUnzipped::at_t<2>, short>);

  // 测试混合容器类型的unzip
  using MixedContainerList =
      tl::type_list<std::tuple<A, int, bool>, tl::type_list<B, double>,
                    std::variant<C, float>>;
  ...
  using HeterogeneousList =
      tl::type_list<std::tuple<A, B>, tl::type_list<C, D>, std::tuple<E, F>>;
...
TEST(type_list, extended_unzip_at) {
  ...
  using TupleList = tl::type_list<std::tuple<A, int, char>,
                                  std::tuple<B, double, short, X, Z>,
  ...
  using VariantList =
      tl::type_list<std::variant<A, int>, std::variant<B, double>,
                    std::variant<C, float>>;
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename VariantUnzippedAt1::at_t<1>, double>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename VariantUnzippedAt1::at_t<2>, float>);

  // 测试混合容器类型的unzip_at
  using MixedList = tl::type_list<std::tuple<A, int>, tl::type_list<B, double>,
                                  std::variant<C, float>>;
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename MixedUnzippedAt1::at_t<1>, double>);
  ...
  using HelperUnzippedAt0 = detail::unzip_at<
      0, tl::type_list<std::tuple<A, int>, tl::type_list<B, double>,
                       std::variant<C, float>>>::type;
...
TEST(type_list, comprehensive_container_operations) {
  ...
  // 2. 对zipped数据进行转换
  using OnlyDatabaseTypes =
      typename ZippedData::transform_t<extract_first_type_t>;
  ...
  using NestedContainerList =
      tl::type_list<std::tuple<tl::type_list<A, B>, int>,
                    std::tuple<tl::type_list<C, D>, double>,
...
TEST(type_list, edge_cases_and_error_handling) {
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZipSingle::at_t<0>, std::tuple<A, int, char>>);
  ...
  using VariantWithDuplicates =
      std::variant<A, A, B>; // 注意：variant可以有重复类型
  using VariantList =
      tl::type_list<VariantWithDuplicates, VariantWithDuplicates>;
  ...
  // 创建包含20个元素的类型列表
  using LargeList =
      tl::type_list<int_type<0>, int_type<1>, int_type<2>, int_type<3>,
                    int_type<4>, int_type<5>, int_type<6>, int_type<7>,
                    int_type<8>, int_type<9>, int_type<10>, int_type<11>,
                    int_type<12>, int_type<13>, int_type<14>, int_type<15>,
                    int_type<16>, int_type<17>, int_type<18>, int_type<19>>;

  using LargeTuple =
      std::tuple<int_type<0>, int_type<1>, int_type<2>, int_type<3>,
                 int_type<4>, int_type<5>, int_type<6>, int_type<7>,
                 int_type<8>, int_type<9>, int_type<10>, int_type<11>,
                 int_type<12>, int_type<13>, int_type<14>, int_type<15>,
                 int_type<16>, int_type<17>, int_type<18>, int_type<19>>;
  ...
  using HelperUnzipAt0 = detail::unzip_at<
      0, tl::type_list<std::tuple<A, int>, std::tuple<B, double>>>::type;
...
TEST(type_list, new_transform_features) {
  ...
  using IndexedList =
      typename SourceList::transform_t<make_indexed_tuple_t,
                                       std::integral_constant<std::size_t, 0>>;

  CHECK_COMPILE_TIME(IndexedList::size == 5);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename IndexedList::at_t<0>,
                     std::tuple<A, std::integral_constant<std::size_t, 0>>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename IndexedList::at_t<4>,
                     std::tuple<E, std::integral_constant<std::size_t, 0>>>);

  // 测试带标签的transform
  struct UserTag {};
  using TaggedList =
      typename SourceList::transform_t<make_tagged_type_t, UserTag>;

  CHECK_COMPILE_TIME(TaggedList::size == 5);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename TaggedList::at_t<0>, std::tuple<A, UserTag>>);
  ...
  using DecoratedList =
      typename SourceList::transform_t<decorate_type_t, Modifier1, Modifier2>;

  CHECK_COMPILE_TIME(DecoratedList::size == 5);
  CHECK_COMPILE_TIME(std::is_same_v<typename DecoratedList::at_t<0>,
                                    std::tuple<A, Modifier1, Modifier2>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedWithIndices::at_t<0>,
                     std::tuple<A, std::integral_constant<std::size_t, 0>>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ZippedWithIndices::at_t<4>,
                     std::tuple<E, std::integral_constant<std::size_t, 4>>>);

  using ModifiedList =
      typename ZippedWithIndices::transform_t<extract_and_modify_t>;

  CHECK_COMPILE_TIME(ModifiedList::size == 5);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ModifiedList::at_t<0>,
                     std::tuple<A, std::integral_constant<std::size_t, 0>,
                                std::integral_constant<std::size_t, 0>>>);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename ModifiedList::at_t<1>,
                     std::tuple<B, std::integral_constant<std::size_t, 1>,
                                std::integral_constant<std::size_t, 2>>>);

  using ConfigResult =
      typename SourceList::transform_t<configurable_transformer_apply_t, int,
                                       double>;
  CHECK_COMPILE_TIME(ConfigResult::size == 5);
  CHECK_COMPILE_TIME(std::is_same_v<typename ConfigResult::at_t<0>,
                                    std::tuple<A, int, double>>);

  // 测试边界情况：空列表的transform
  using EmptyList = tl::type_list<>;
  using TransformedEmpty =
      typename EmptyList::transform_t<make_indexed_tuple_t, int>;
  CHECK_COMPILE_TIME(TransformedEmpty::size == 0);

  // 测试单元素列表的transform
  using SingleList = tl::type_list<A>;
  using TransformedSingle =
      typename SingleList::transform_t<make_indexed_tuple_t, int>;
  CHECK_COMPILE_TIME(TransformedSingle::size == 1);
  CHECK_COMPILE_TIME(
      std::is_same_v<typename TransformedSingle::at_t<0>, std::tuple<A, int>>);

  // 测试transform链式调用
  using FirstTransform =
      typename SourceList::transform_t<make_tagged_type_t, int>;
  using SecondTransform =
      typename FirstTransform::transform_t<make_tagged_type_t, double>;
  CHECK_COMPILE_TIME(SecondTransform::size == 5);
  CHECK_COMPILE_TIME(std::is_same_v<typename SecondTransform::at_t<0>,
                                    std::tuple<std::tuple<A, int>, double>>);
  ...
  // 验证FieldInfo是否被正确添加
  CHECK_COMPILE_TIME(std::is_same_v<typename WithFieldInfo::at_t<0>,
                                    std::tuple<A, field_info>>);
  ...
  using WithSerialization =
      typename SourceList::transform_t<serialization_info_t>;
  ...
  using ConditionalResult =
      typename SourceList::transform_t<conditional_transform_t>;
...
TEST(type_list, indexed_transform) {
  // 创建索引列表的辅助函数
  constexpr auto make_index_list =
  ...
  using SourceList = tl::type_list<A, B, C, D, E>;
  using Indices =
      decltype(make_index_list(std::make_index_sequence<SourceList::size>{}));
  ...
  CHECK_COMPILE_TIME(SecondOffset::value ==
                     sizeof(A)); // B的偏移量应该是A的大小

  // 测试更复杂的索引计算
  // 需要累积偏移量的计算
  using OffsetsList = tl::type_list<
      std::integral_constant<std::size_t, 0>,
  ...
  CHECK_COMPILE_TIME(LastOffset::value ==
                     sizeof(A) + sizeof(B) + sizeof(C) + sizeof(D));
...
TEST(type_list, from_function) {
  ...
  using Fn1 = void(int);
  CHECK_COMPILE_TIME(
      std::is_same_v<tl::empty_list::from_function_t<Fn1>, tl::type_list<int>>);

  // 测试2：函数指针（void(*)(int, float)）
  using Fn2 = void (*)(int, float);
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn2>,
                                    tl::type_list<int, float>>);

  // 测试3：函数引用（void(&)(double)）
  using Fn3 = void (&)(double);
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn3>,
                                    tl::type_list<double>>);
  ...
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn4>,
                                    tl::type_list<std::string>>);
  ...
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn5>,
                                    tl::type_list<bool>>);

  auto lambda = [](char, short) {};
  using Fn6 = decltype(lambda);
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn6>,
                                    tl::type_list<char, short>>);

  auto generic_lambda = []<typename T>(T) {};
  using Fn7 = decltype(generic_lambda);
  // 对于泛型lambda，我们无法直接推断参数类型，因此from_function_t应该返回一个空列表
  CHECK_COMPILE_TIME(
      std::is_same_v<tl::empty_list::from_function_t<Fn7>, tl::type_list<>>);
  ...
  CHECK_COMPILE_TIME(
      std::is_same_v<tl::empty_list::from_function_t<Fn8>, tl::type_list<>>);

  auto func = std::function<void(float, double)>{};
  using Fn9 = decltype(func);
  // std::function的参数类型应该能被正确提取
  CHECK_COMPILE_TIME(std::is_same_v<tl::empty_list::from_function_t<Fn9>,
                                    tl::type_list<float, double>>);

  // 测试边界情况：函数类型没有参数
  using Fn10 = void();
  CHECK_COMPILE_TIME(
      std::is_same_v<tl::empty_list::from_function_t<Fn10>, tl::type_list<>>);
  return true;
}
