...
template <class... Ts> struct type_list {
  ...
  template <class T> using contains = detail::contains<T, type>;
  template <class T> static constexpr bool contains_v = contains<T>::value;

  template <class T> static constexpr bool none_of = !contains_v<T>;
  template <class T> static constexpr bool any_of = contains_v<T>;
  template <class T>
  ...
  static constexpr std::size_t count_of =
      count_if<detail::is_same<T>::template impl>;

  template <std::size_t I>
  // using at = std::tuple_element<I, std::tuple<Ts...>>;
  using at = detail::select<type, I>;
  template <std::size_t I> using at_t = typename at<I>::type;

  template <template <class...> class Fn> using apply = Fn<Ts...>;
  template <template <class...> class Fn>
  using apply_t = typename apply<Fn>::type;

  template <std::size_t Start, std::size_t Length>
  using slice = std::conditional_t<Length == 0 || is_empty, type_list<>,
                                   detail::slice<type, Start, Length>>;
  template <std::size_t Start, std::size_t Length>
  using slice_t = typename slice<Start, Length>::type;

  template <std::size_t N> using head = slice<0, N>;
  template <std::size_t N> using head_t = typename head<N>::type;

  template <std::size_t N> using tail = slice<size - N, N>;
  template <std::size_t N> using tail_t = typename tail<N>::type;

  template <class... Us> using append = type_list<Ts..., Us...>;
  template <class... Us> using append_t = typename append<Us...>::type;

  template <class... Us> using prepend = type_list<Us..., Ts...>;
  template <class... Us> using prepend_t = typename prepend<Us...>::type;
  ...
  template <class... Us> using rebind = type_list<Us...>;
  ...
  using replace =
      type_list<std::conditional_t<std::is_same_v<Ts, From>, To, Ts>...>;
  template <class From, class To>
  ...
  template <std::size_t I> using remove_at = detail::remove_at<type, I>;
  template <std::size_t I> using remove_at_t = typename remove_at<I>::type;
  ...
  template <class T> using remove_t = typename remove<T>::type;
  ...
  static constexpr std::size_t find_last_not_if = [] {
    constexpr std::size_t index =
        type::reverse_t::template find_first_not_if<Pred>;
    return index == npos ? npos : type::size - 1 - index;
  }();

  template <class T>
  static constexpr std::size_t index_of =
      find_first_if<detail::is_same<T>::template impl>;
  ...
  static constexpr bool is_subset_of =
      []<std::size_t... Is>(std::index_sequence<Is...>) {
        if constexpr (size > List::size) {
          ...
          return ((List::template contains_v<at_t<Is>>) && ...);
        }
      }(Indices{});
  ...
  template <class List> using union_with_t = typename union_with<List>::type;

  template <class List> using difference_with = reject<List::template contains>;
  template <class List>
  using difference_with_t = typename difference_with<List>::type;

  template <class List> using intersect_with = filter<List::template contains>;
  template <class List>
  using intersect_with_t = typename intersect_with<List>::type;

  template <class Other> using concat = detail::concat<type, Other>;
  template <class Other> using concat_t = typename concat<Other>::type;

  using flatten = detail::flatten<type>;
  using flatten_t = typename flatten::type;

  template <class... Us>
  using zip = std::conditional_t<
      is_empty, type_list<>,
      std::conditional_t<detail::all_are_container_v<Us...>,
                         detail::zip<type, Us...>,
                         detail::zip<type, type_list<Us...>>>>;
  template <class... Us> using zip_t = typename zip<Us...>::type;

  using unzip =
      std::conditional_t<(is_empty || !detail::all_are_container_v<Ts...> ||
  ...
  using unzip_at =
      std::conditional_t<(is_empty || !detail::all_are_container_v<Ts...>),
                         type_list<>, detail::unzip_at<I, type>>;
  template <std::size_t I> using unzip_at_t = typename unzip_at<I>::type;
  ...
  static constexpr void reverse_for_each(F &&f, Args &&...args) {
    if constexpr (size > 0) {
      [&]<size_t... Is>(std::index_sequence<Is...>) {
        (std::forward<F>(f)(detail::identity<at_t<size - 1 - Is>>{},
  ...
  static constexpr auto dispatch_at(F &&f, Args &&...args) {
    ...
    return std::invoke(std::forward<F>(f), detail::identity<at_t<I>>{},
                       std::forward<Args>(args)...);
  }
