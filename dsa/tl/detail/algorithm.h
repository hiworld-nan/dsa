...
static constexpr bool is_container_v =
    container_traits<Container>::is_container;

template <class Container>
static constexpr std::size_t container_size_v =
    container_traits<Container>::size;
...
static constexpr std::size_t min_container_size_v = [] {
  ...
  if constexpr (valid) {
    ...
    ((min = (container_size_v<Containers> < min) ? container_size_v<Containers>
                                                 : min),
     ...);
    return min;
  } else {
...
template <class T, class Container> struct contains {
private:
  static_assert(is_container_v<Container>,
                "contains requires a type_list/tuple/variant");

  template <class> struct impl;

  template <class... Ts> struct impl<tl::type_list<Ts...>> {
...
private:
  static_assert(
      is_container_v<L> && is_container_v<R>,
...
public:
  using type = typename convert_to<typename concat<left, right>::type,
                                   container_t<L>>::type;
};

template <class L, class R> using concat_t = typename concat<L, R>::type;

template <class... Containers> struct concat_all {
private:
  static_assert(sizeof...(Containers) > 0,
                "concat_all requires at least one list");
  static_assert(
      all_are_container_v<Containers...>,
      "concat_all requires all parameters to be type_list/tuple/variant");

  template <class... Lists> struct impl;
...
template <class Container> struct reverse {
private:
  static_assert(is_container_v<Container>,
                "reverse requires a type_list/variant/tuple");

  template <std::size_t... Is>
  static constexpr auto impl(std::index_sequence<Is...>)
      -> tl::type_list<
          element_t<Container, container_size_v<Container> - 1 - Is>...>;
...
template <class U> struct remove {
  template <class Acc, class T> struct impl {
    using type = std::conditional_t<std::is_same_v<U, T>, Acc,
                                    typename Acc::template append<T>>;
  };
};

template <class Acc, class T> struct unique {
  using type = std::conditional_t<Acc::template contains_v<T>, Acc,
                                  typename Acc::template append<T>>;
};

template <template <class> class Pred> struct filter {
  template <class Acc, class T> struct impl {
    using type = std::conditional_t<Pred<T>::value,
                                    typename Acc::template append<T>, Acc>;
  };
};

template <template <class> class Pred> struct reject {
  template <class Acc, class T> struct impl {
    using type = std::conditional_t<Pred<T>::value, Acc,
                                    typename Acc::template append<T>>;
  };
};

template <class Container, std::size_t I> struct select {
  static_assert(is_container_v<Container>,
                "select requires a type_list/variant/tuple");
  static_assert(I < container_size_v<Container>,
                "Index out of range for select");
...
template <class List, std::size_t Start, std::size_t Length> struct slice {
private:
  static_assert(is_container_v<List>,
                "slice requires a type_list/variant/tuple");
  static_assert(Start + Length <= container_size_v<List>,
                "Slice Start + Length out of range");

  template <std::size_t... Is>
  static auto impl(std::index_sequence<Is...>)
      -> tl::type_list<element_t<List, Start + Is>...>;
...
struct accumulate {
private:
  template <class Acc, class... Rest> struct impl;

  template <class Acc> struct impl<Acc> {
    using type = Acc;
  };

  template <class Acc, class T> struct impl<Acc, T> {
...
template <class List, std::size_t I, class T> struct replace_at {
private:
  static constexpr std::size_t size = container_size_v<List>;

  static_assert(is_container_v<List>,
                "replace_at requires type_list/tuple/variant");
...
template <class List, std::size_t I, class T> struct insert_at {
private:
  static constexpr std::size_t size = container_size_v<List>;

  static_assert(I <= size, "Index out of range for insert_at");
  static_assert(is_container_v<List>,
                "insert_at requires type_list/tuple/variant");
...
template <class List, std::size_t I> struct remove_at {
private:
  static constexpr std::size_t size = container_size_v<List>;

  static_assert(I < size, "Index out of range for remove_at");
  static_assert(is_container_v<List>,
                "remove_at requires type_list/tuple/variant");
...
template <class List, class... Containers> struct zip {
private:
  static constexpr std::size_t min_size =
      min_container_size_v<List, Containers...>;
  ...
  using zip_element_t =
      std::tuple<element_t<List, I>, element_t<Containers, I>...>;

  template <std::size_t... Is>
  static auto
      impl(std::index_sequence<Is...>) -> tl::type_list<zip_element_t<Is>...>;
...
template <std::size_t I, class List> struct unzip_at {
  template <std::size_t, class> struct impl;

  template <std::size_t Idx, class... Containers>
  struct impl<Idx, tl::type_list<Containers...>> {
    static constexpr std::size_t size = sizeof...(Containers);
    static constexpr std::size_t min_container_size =
        min_container_size_v<Containers...>;
...
template <class List> struct unzip {
private:
  template <class> struct impl;

  template <class... Containers> struct impl<tl::type_list<Containers...>> {
  private:
    static constexpr std::size_t size = sizeof...(Containers);
    static constexpr std::size_t min_container_size =
        min_container_size_v<Containers...>;

    static_assert(size && min_container_size,
                  "unzip requires at least one tuple");
    ...
    template <std::size_t... Is>
    static auto
        get(std::index_sequence<Is...>) -> std::tuple<unzip_element_t<Is>...>;
...
template <class... Ts> struct flatten<tl::type_list<Ts...>> {
private:
  template <class Us> struct flatten_with {
    using type = tl::type_list<Us>;
  };

  template <class... Us> struct flatten_with<tl::type_list<Us...>> {
    using type = typename flatten<tl::type_list<Us...>>::type;
  };

  template <class... Us> struct flatten_with<std::variant<Us...>> {
    using type = typename flatten<tl::type_list<Us...>>::type;
  };

  template <class... Us> struct flatten_with<std::tuple<Us...>> {
    using type = typename flatten<tl::type_list<Us...>>::type;
  };

  template <class List, class... Rest> struct impl;

  template <class... Us> struct impl<tl::type_list<Us...>> {
    using type = tl::type_list<Us...>;
  };

  template <class... Us, class T> struct impl<tl::type_list<Us...>, T> {
  private:
