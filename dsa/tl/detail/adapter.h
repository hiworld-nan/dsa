...
template <class... Ts> struct container_traits<tl::type_list<Ts...>> {
  using type = tl::type_list<Ts...>;
  static constexpr bool is_container = true;
  static constexpr size_t size = sizeof...(Ts);

  template <size_t I> using at = std::tuple_element_t<I, std::tuple<Ts...>>;
};

template <class... Ts> struct container_traits<std::tuple<Ts...>> {
  using type = std::tuple<Ts...>;
  static constexpr bool is_container = true;
  static constexpr size_t size = sizeof...(Ts);

  template <size_t I> using at = std::tuple_element_t<I, type>;
};

template <class... Ts> struct container_traits<std::variant<Ts...>> {
...
template <class, class> struct convert_to;

template <template <class...> class From, class... Ts,
          template <class...> class To, class... Us>
...
private:
  ...
  template <class R, class Class, class... Params>
  static auto
      impl(R (Class::*)(Params...) volatile) -> tl::type_list<Params...>;

  template <class R, class Class, class... Params>
  static auto
      impl(R (Class::*)(Params...) noexcept) -> tl::type_list<Params...>;

  template <class Callable>
  static auto
  impl(Callable &&) -> decltype(impl(&std::decay_t<Callable>::operator()));
